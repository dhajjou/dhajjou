# Doha HAJJOU

La traduction française est ci-dessous. 👇

🎓 Master's degree in Data Engineering from EFREI Paris.<br>
💼 Data Engineer specialized in data integration, modeling, and pipeline automation with Azure, Snowflake, Talend, and Python.<br>
🔍 **Available immediately for a permanent position (CDI).**<br>
📧 doha.hajjou@gmail.com  
📞 +33 751 26 99 68  
📍 Paris, Île-de-France

----------------------------------------------------------------------------------------------------

🚀 Welcome to my GitHub repository! Here you'll find a selection of my projects in Data Engineering, Cloud, and ETL automation. 
Each project reflects my focus on building reliable, scalable, and efficient data solutions.

###![Doha's GitHub stats](https://github-readme-stats.vercel.app/api?username=dhajjou&show_icons=true&theme=default)

----------------------------------------------------------------------------------------------------

## 💼 Experience

**Manutan – Data Engineer**  
📍 Gonesse, Île-de-France  
📅 Sep 2022 – Present

- 🧪 Designed and optimized data pipelines using Talend, Snowflake, and Azure Data Factory
- 🧪 Developed automated tests to ensure data quality and reliability
- 📥 Extracted and loaded XML data into Snowflake  
- 📊 Mapped and analyzed customer/sales data from diverse sources  
- 📘 Documented data governance connectors and profiled data with Informatica  
- 📐 Created interface contracts and data models using PowerDesigner  
- 🧠 Built and evaluated predictive models for web order behavior (Python, Pandas)  
- 📅 Estimated workloads and planned deliverables using Microsoft Project  
- 📄 Drafted technical and functional specifications for multiple projects  

**Full Stack Developer Intern – DPETLE**  
📍 Settat, Morocco  
📅 Jul 2021 – Sep 2021  
- 👩‍💻 Built an intern management app using Java & MySQL  
- 🖥️ Diagnosed and configured workstations  

**Full Stack Developer Intern – RADEEC**  
📍 Settat, Morocco  
📅 Jul 2019 – Sep 2019  
- 📦 Built a stock management tool with Excel/VBA  
- 📝 Analyzed project files and corrected application bugs
  
----------------------------------------------------------------------------------------------------

## 🎓 Education

🎓 **EFREI Paris**  
Master’s in Data Engineering (Sep 2022 – Sep 2024)  
[Program link](https://www.efrei.fr/wp-content/uploads/2022/02/Mastere-Data-Engineering-1.pdf)

🎓 **EMSI Casablanca**  
Engineering Degree in Computer Science (Sep 2020 – Jul 2021)

🎓 **BTS Settat**  
Diploma in Information Systems Development (Sep 2017 – Jul 2018)

----------------------------------------------------------------------------------------------------

## 🧾 Certifications

- ✅ Microsoft Certified: Azure Data Engineer Associate (DP-203)
- ✅ Microsoft Certified: Azure Fundamentals (AZ-900)
- 📚 Udemy:  
  - Big Data & Data Science  
  - Snowflake – Complete Guide  
  - Data Governance Fundamentals  
  - Talend for ETL/BI

----------------------------------------------------------------------------------------------------

## 🛠 Technical Skills

- **Languages:** SQL (T-SQL, PL/SQL), Python , Java, C, C++, C#, VBA, Visual Basic  
- **Web:** HTML5, CSS3, JavaScript, PHP, Bootstrap  
- **Databases:** Snowflake, SQL Server, PostgreSQL, MongoDB, MySQL  
- **ETL & BI:** Talend, Informatica  
- **Cloud & Tools:** Microsoft Azure, Git, GitHub, GitLab  
- **Modeling:** UML, Merise, PowerDesigner  
- **Methods:** Agile, Scrum  
- **Soft Skills:** Organization, teamwork, autonomy, analytical thinking, adaptability

----------------------------------------------------------------------------------------------------

## 🌍 Languages

🇫🇷 French – Fluent  
🇬🇧 English – Fluent  
🇦🇪 Arabic – Native

----------------------------------------------------------------------------------------------------

## 🎨 Personal Interests

Reading · Music · Horseback riding · Painting · Interior Design

----------------------------------------------------------------------------------------------------

Version Française

----------------------------------------------------------------------------------------------------


# Doha HAJJOU

🎓 Diplômée d’un Mastère en Ingénierie des Données à EFREI Paris.<br>
💼 Data Engineer spécialisée en intégration, modélisation et automatisation de pipelines sur Azure, Snowflake, Talend et Python.<br>
🔍 **Disponible immédiatement pour un poste en CDI.**<br>
📧 doha.hajjou@gmail.com  
📞 +33 751 26 99 68  
📍 Paris, Île-de-France

----------------------------------------------------------------------------------------------------

🚀 Bienvenue sur mon profil GitHub !  
Vous trouverez ici une sélection de mes projets en Data Engineering, Cloud et automatisation de flux de données.  
Chaque projet illustre ma volonté de concevoir des solutions fiables, performantes et évolutives.

----------------------------------------------------------------------------------------------------

## 💼 Expériences

**Manutan – Data Engineer (en alternance)**  
📍 Gonesse, Île-de-France  
📅 Septembre 2022 – Présent 

- 🧪 Conception et optimisation de pipelines de données avec Talend, Snowflake et Azure Data Factory
- 🧪 Développement de tests automatisés pour assurer la qualité et la cohérence des données
- 📥 Extraction et chargement de données XML dans Snowflake  
- 📊 Analyse et cartographie des données clients et ventes provenant de sources multiples  
- 📘 Documentation des connecteurs de gouvernance des données et profiling avec Informatica  
- 📐 Création de contrats d’interface et de modèles de données via PowerDesigner  
- 🧠 Création et évaluation de modèles prédictifs liés aux comportements de commande en ligne (Python, Pandas)  
- 📅 Estimation des charges et planification des livrables (Microsoft Project)  
- 📄 Rédaction des spécifications techniques et fonctionnelles pour divers projets  

**Stagiaire – Développeuse Full Stack (DPETLE)**  
📍 Settat, Maroc  
📅 Juillet 2021 – Septembre 2021  
- 👩‍💻 Développement d’une application de gestion des stagiaires (Java & MySQL)  
- 🖥️ Diagnostic et configuration des postes de travail  

**Stagiaire – Développeuse Full Stack (RADEEC)**  
📍 Settat, Maroc  
📅 Juillet 2019 – Septembre 2019  
- 📦 Développement d’un outil de gestion de stock avec Excel/VBA  
- 📝 Analyse des dossiers projets et correction des bugs applicatifs


----------------------------------------------------------------------------------------------------


## 🎓 Formation

🎓 **EFREI Paris**  
Mastère en Ingénierie des Données (Septembre 2022 – Septembre 2024)  
[Lien vers le programme](https://www.efrei.fr/wp-content/uploads/2022/02/Mastere-Data-Engineering-1.pdf)

🎓 **EMSI Casablanca**  
Diplôme d'ingénieur en Informatique (Septembre 2020 – Juillet 2021)

🎓 **BTS Settat**  
BTS en Développement des Systèmes d’Information (Septembre 2017 – Juillet 2018)

----------------------------------------------------------------------------------------------------

## 🧾 Certifications

- ✅ Microsoft Certified : Azure Data Engineer Associate (DP-203)
- ✅ Microsoft Certified : Azure Fundamentals (AZ-900)
- 📚 Udemy :  
  - Big Data & Data Science  
  - Snowflake – Guide Complet  
  - Fondamentaux de la Data Governance  
  - Talend pour ETL/BI
 
----------------------------------------------------------------------------------------------------

## 🛠 Compétences Techniques

- **Langages :** SQL (T-SQL, PL/SQL), Python, Java, C, C++, C#, VBA, Visual Basic  
- **Web :** HTML5, CSS3, JavaScript, PHP, Bootstrap  
- **Bases de données :** Snowflake, SQL Server, PostgreSQL, MongoDB, MySQL  
- **ETL & BI :** Talend, Informatica  
- **Cloud & Outils :** Microsoft Azure, Git, GitHub, GitLab  
- **Modélisation :** UML, Merise, PowerDesigner  
- **Méthodes :** Agile, Scrum  
- **Soft Skills :** Organisation, travail en équipe, autonomie, adaptabilité, esprit analytique


----------------------------------------------------------------------------------------------------

## 🌍 Langues

🇫🇷 Français – Courant  
🇬🇧 Anglais – Courant  
🇦🇪 Arabe – Langue maternelle  

----------------------------------------------------------------------------------------------------

## 🎨 Centres d’intérêt

Lecture · Musique · Équitation · Peinture · Décoration d’intérieur
